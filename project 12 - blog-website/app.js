...
app.get("/posts/:p", function(req, res) {
  ...
  Post.findOne({title:requestedtitle}, function(err,foundpost){
    res.render("post", {mypost: foundpost});
  });
});

app.post("/compose", function(request, response) {
  const title = _.lowerCase(request.body.posttitle);
  const text = request.body.posttext;

  const post = new Post({ //this is a javascript object
    title: title,
    text: text
  });
  post.save();
